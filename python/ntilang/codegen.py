...
class Emitter:
    ...
    def expression(self, expr: Expr):
        ...
        if op in FAST_MATH_OPS:
            ...
            if operation in ("exp10", "log2", "log10") or (operation == "exp" and dtype == "float16"):
                return self.math_low_precision(f"fast_{operation}", dtype, values)
            # math.h aliases exp/log/sin/cos to CUTLASS, whose low-precision
            # wrappers use ordinary FP32 libdevice here. common.h does the same for tan.
            widened = self.math_external(f"__nv_{operation}f", "float32", values, ("float32",))
