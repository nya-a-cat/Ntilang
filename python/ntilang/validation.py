...
def validate(kernel: Kernel):
    ...
    def record_write(name, indices, bounds, definitions, in_serial):
        if in_serial:
            raise CompileError("Write global outputs after serial tile accumulation loops")
        if name in writes:
            raise CompileError(
                f"Output {name} has multiple write sites; use a temporary tile for intermediate results"
            )
        check_ownership(indices, bounds, definitions)
        writes.add(name)

    def statements(body, bounds, definitions, in_serial=False):
        for stmt in body:
